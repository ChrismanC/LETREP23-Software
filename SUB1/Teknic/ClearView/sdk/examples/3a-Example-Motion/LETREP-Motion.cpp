...
//need pipe libraries from win32

//before inter-process communication:
//create pipe on Python end
//connect to it on C end
//create counter-pipe on C end
//connect to it on Python end
//name these pipes' handles so that you can tell which sends and which receives on each end
// 
//messages sent will force the process to wait until they are received!!!!!
//therefore, make sure sent messages from one end can expect to be read on the other 
//OR check documentation on asyncronous win32 pipes and change everything.
// 
//if this requirement is satisfied, messages can be freely sent and received so long as the pipe exists. See examples; server sends to client
// 
//when the program ends, please close pipe handles on both ends
//
